...
actual fun Detail(
    ...
    Div {
        TopBar(note = note, onClose = onClose, onSave = { /*vm.onSave()*/ }, onDelete = { /*vm.onDelete()*/ })
...
private fun TypeDropdown(value: Note.Type, onValueChange: (Note.Type) -> Unit, modifier: Modifier) {
    Select(
        attrs = {
            classes(AppStyleSheet.detailInput)
            onChange {
                onValueChange(Note.Type.valueOf(it.target.value))
            }
        }
    ) {
...
private fun TopBar(note: Note, onClose: () -> Unit, onSave: () -> Unit, onDelete: () -> Unit) {
    Div(
        ...
        Icon(
            iconName = "close",
            attrs = { classes(AppStyleSheet.topBarIcon); onClick { onClose() } }
        )
        ...
        Icon(
            iconName = "save",
            attrs = { classes(AppStyleSheet.topBarIcon); onClick { onSave() } }
        )
        if (note.id != Note.NEW_NOTE_ID){
            Icon(
                iconName = "delete",
                attrs = { classes(AppStyleSheet.topBarIcon) ; onClick { onDelete() } }
            )
        }
    }
}